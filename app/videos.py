...
class Videos:
        ...
        if video is not None and videoname == video.video_name:
            if eventname is not None:
                ...
                if ev is not None:
                    ...
                    ev2 = models.EventsData(
                        eventname,
                        date_added=now,
                        date_updated=now)
                    db.session.add(ev2)
                    db.session.commit()
                    ev2 = models.EventsData.query.filter_by(
                        event_name=eventname).first()
                    video = models.VideosData(
                        videoname,
    ...
    def getCategories(self):
        data = db.session.query(
            models.VideosData.category.distinct().label("category")).all()
        results = {}
        if data:
    ...
    def getTypes(self):
        data = db.session.query(
            models.VideosData.type.distinct().label("type")).all()
        results = {}
        if data:
            for row, value in enumerate(data):
                results[row] = {
