...
class Images:
    def getImageNames(self):
        ...
        if category is not None and int(category) == 1:
            ...
            if event is not None:
                ev = models.EventsData.query.filter_by(
                    event_name=event).first()
                images = models.ImagesData.query.filter_by(
                    event_id=ev.event_id).all()
        ...
        for i, key in enumerate(bucket.list()):
            if key.name == filename:
                ...
                try:
                    ...
                    return Response(
                        e.body, status=e.status, headers=key.resp.getheaders())
    ...
    def addImage(self,
        ...
        for data_file in image_files:
            ...
            if sent == size:
                self.addImageToDb(k.key, video_id=video_id,
                                  eventname=eventname,
                                  category=category,
                                  type=type)
    ...
    def addImageToDb(self,
        ...
        if img is not None and imagename == img.image_name:
            ...
            if eventname is not None:
                ev = models.EventsData.query.filter_by(
                    event_name=eventname).first()
                img = models.ImagesData(
                    imagename,
                    video_id=video_id, event_id=ev.event_id,
                    event_name=ev.event_name, date_added=now,
                    date_updated=now)
            else:
                print "No event name provided"
                img = models.ImagesData(
                    imagename, video_id=video_id,
